...
# Step 1: Extract embeddings from TabNet model
clf.network.eval()
...
# Set colorbar ticks and labels 1 to 16
unique_labels = np.unique(ytest)
cbar = plt.colorbar(scatter, ticks=unique_labels)
##cbar.set_label('Class Label', fontsize=10)
cbar.set_ticklabels([str(int(i + 1)) for i in unique_labels])

# Clean plot: no ticks, no box
plt.xticks([])
plt.yticks([])
plt.box(False)
##plt.title("t-SNE Visualization of TabNet Embeddings (Indian Pines)", fontsize=12)

# Save high-quality PNG
plt.tight_layout()
plt.savefig("tabnet_tsne_visualization_indian_classmapcolor_VARYTrain.png", dpi=300)
plt.close()
