...
class TabNetEncoder(torch.nn.Module):
    def __init__(
        ...
        # Placeholder linear layer (initialized dynamically on first forward pass)
        self.olinear = None
    ...
    def forward(self, x, prior=None):
        ...
        for step in range(self.n_steps):
            ...
            #M_reshaped = M.reshape(M.shape[0], 10, 15, 15)
            ...
            #orelthree = self.cbam(orelthree)
                ...
                # Convolutional processing with CBAM
            out_conv = self.conv1(M_reshaped)
            ...
            out_conv = self.conv3(out_conv)
            out_conv = self.bn3(out_conv)
            #out_conv = F.gelu(out_conv)
            out_conv = F.relu(out_conv)
           ...
           ## Dynamic Feature Recalibration via Squeeze-and-Excitation (SE) for Tabular Features
...
class FeatTransformer(torch.nn.Module):
    def __init__(
        ...
        if shared_layers is None:
            ...
            #    input_dim=input_dim,
            #    output_dim=output_dim,
            #    n_blocks=len(shared_layers),  # treat each shared_layer as a block
            #    dropout=0.0  # you can tune this
            #)
            #is_first = False
            #self.shared = AGLU_Block(
            #    input_dim=input_dim,
            #    output_dim=output_dim,
            #    n_blocks=len(shared_layers),  # match shared_layers depth
            #    dropout=0.1
            #)
            #is_first = False
    ...
    def forward(self, x):			
        ...
        print(' shared shape',x_shared.shape)
        x_specific = self.specifics(x)
        print(' specific shape',x_specific.shape)

        # Fusion mechanism here
