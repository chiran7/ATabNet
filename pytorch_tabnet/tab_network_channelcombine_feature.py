...
class FeatTransformer(nn.Module):
    def __init__(
        self,
        shared_layers,           # <-- moved to first position for clarity
        input_dim,
        output_dim,
        n_glu_independent,
        virtual_batch_size=128,
        ...
        """
        Feature transformer block consisting of shared + specific GLU blocks,
        followed by fusion.

        Parameters:
        -----------
        shared_layers : nn.ModuleList or None
            Shared linear layers across steps.
        input_dim : int
        ...
        self.fuse = FuseLayer(dim=output_dim)
        ...
        if shared_layers is not None:
            self.shared = GLU_Block(
                ...
                output_dim=output_dim,
                first=is_first,
                shared_layers=shared_layers,               # ? correct argument
                n_glu=len(shared_layers),                 # ? fixed: get length of list
                virtual_batch_size=virtual_batch_size,
                momentum=momentum,
            )
            is_first = False  # only first block uses initial input dim

        # Specific (independent) GLU block
