...
class TabModel(BaseEstimator):
    ...
    def _train_batch(self, X, y):
        ...
        # print('output label shape',output.shape)
        # print('actual label shape',y.shape)		

        #output, M_loss, masks = self.network(X, return_masks=True)
        ...
        # Inside _train_batch
        entropy_reg = self.entropy_loss(masks)  # assuming M_loss is mask output, otherwise get mask from network
        loss = self.compute_loss(output, y)
        loss += self.lambda_sparse * M_loss  # existing sparsity
