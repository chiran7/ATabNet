...
class TabModel(BaseEstimator):
    ...
    def fit(
        ...
        if loss_fn is None:
            ...
            print(f"calling executing of new loss function")
    ...
    def _train_epoch(self, train_loader):
        ...
        return
    def entropy_loss(self, masks, epsilon=1e-15):
        """
        Compute entropy loss for a dict of mask tensors.

        masks : dict of tensors, each of shape (batch_size, num_features)
        """
        total_entropy = 0.0
        count = 0

        for key, mask in masks.items():
            # mask shape expected (batch_size, features)
            if mask.dim() == 1:
                # unlikely, but just in case
                entropy = -(mask * torch.log(mask + epsilon))
            else:
                entropy = -torch.sum(mask * torch.log(mask + epsilon), dim=1)
    ...
    def focal_loss(self, inputs, targets, alpha=0.25, gamma=2):
        ...
        targets: one-hot encoded or class indices (batch_size) - convert if needed
        """
        ...
        pt = torch.exp(-BCE_loss)  # pt = sigmoid probability of true class
        loss = alpha * (1 - pt) ** gamma * BCE_loss

        return loss.mean()

    def label_smoothing_loss(self, preds, targets, smoothing=0.1):
    ...
    def mask_l1_loss(self, masks):
        total_l1 = 0
        for _, mask in masks.items():
            total_l1 += torch.mean(torch.abs(mask))
        return total_l1 / len(masks)
    ...
    def _train_batch(self, X, y):
        ...
        output, M_loss = self.network(X)  # output shape (batch_size, num_classes)

        # Compute focal loss instead of cross entropy
        #loss = self.focal_loss(output, y, alpha=0.25, gamma=2)
        loss = self.label_smoothing_loss(output, y, smoothing=0.1)
        #loss = self.compute_loss(output, y)
        

        self.lambda_l1 = 1e-3
        # Add your sparsity and entropy regularizations
        #loss += self.lambda_sparse * M_loss

        M_explain, masks = self.network.forward_masks(X)
        loss += self.lambda_sparse * M_loss + self.lambda_l1 * self.mask_l1_loss(masks)

        ##this with lambda_sparse gave 97.27 ind pines
        if hasattr(self, 'lambda_entropy') and self.lambda_entropy > 0:
            #M_explain, masks = self.network.forward_masks(X)
            entropy_reg = self.entropy_loss(masks)
            loss += self.lambda_entropy * entropy_reg
    ...
    def cosine(self, y_score, y_true):
    ...
    # Compute the second loss, 1 - cos
        ...
        self.loss=torch.mean(loss_1 + loss_2)
        return
